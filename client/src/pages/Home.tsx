...
function LegacyProductVisual({ product, detail = false, compact = false }: { product: Product; detail?: boolean; compact?: boolean }) {
  ...
  return <div className={`${detail ? "detail-visual" : compact ? "solution-signal-icon" : "product-image-wrap"} product-${product.slug}`}>
    <div className="visual-fallback" role="img" aria-label={visualLabel} data-visual-source="Eryndex product signal diagram"><img className="product-visual-image" src={product.image} alt="" aria-hidden="true" /><div className="product-visual-wash" aria-hidden="true" /><svg className="product-motion-overlay" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
...
function ProductSatelliteSystem({ lang }: { lang: Lang }) {
  ...
  return <div className="product-network">
    ...
    <div className="network-core"><img src="https://nexoracorp-mfb35dfs.manus.space/manus-storage/eryndex-logo-light-signal_53d6e599.png" alt="Eryndex" /><span>{lang === "en" ? "One connected system" : lang === "zh-CN" ? "一个连接的系统" : "一個連結的系統"}</span></div>
