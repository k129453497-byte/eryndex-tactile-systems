...
const products = [
  {
    ...
    image: "/product-visuals/qadryn-flow.png",
    icon: Workflow,
    tone: "cyan",
  },
  {
    ...
    image: "/product-visuals/vessyra-boundary.png",
    icon: ShieldCheck,
    tone: "mint",
  },
  {
    ...
    image: "/product-visuals/mireqon-context.png",
    icon: BarChart3,
    tone: "violet",
  },
  {
    ...
    image: "/product-visuals/terviq-local-intelligence.png",
    icon: Cpu,
    tone: "cyan",
  },
] as const;
...
function LegacyProductVisual({ product, detail = false, compact = false }: { product: Product; detail?: boolean; compact?: boolean }) {
  ...
  return <div className={`${detail ? "detail-visual" : compact ? "solution-signal-icon" : "product-image-wrap"} product-${product.slug}`}>
    <div className="visual-fallback" role="img" aria-label={visualLabel} data-visual-source="Eryndex product signal diagram"><img className="product-visual-image" src={publicAsset(product.image)} alt="" aria-hidden="true" /><div className="product-visual-wash" aria-hidden="true" /><svg className="product-motion-overlay" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
...
function ProductSatelliteSystem({ lang }: { lang: Lang }) {
  ...
  const paths = ["M 270 145 C 390 145 390 270 500 270", "M 730 145 C 610 145 610 270 500 270", "M 270 395 C 390 395 390 270 500 270", "M 730 395 C 610 395 610 270 500 270"];
  return <div className="product-network">
    <svg className="network-connections" viewBox="0 0 1000 540" preserveAspectRatio="none" aria-hidden="true">{paths.map((path, i) => <g key={path} className={"network-route route-" + i}><path d={path} /><path className="network-signal" d={path} pathLength="100" style={{ animationDelay: "-" + i * 1.2 + "s" }} /></g>)}</svg>
    <div className="network-core"><img src={publicAsset("brand/eryndex-logo.webp")} alt="Eryndex" /><span>{lang === "en" ? "One connected system" : lang === "zh-CN" ? "一个连接的系统" : "一個連結的系統"}</span></div>
    {products.map((product, i) => <Link key={product.slug} href={"/products/" + product.slug} className={"network-product network-position-" + i}><div className="network-product-media"><LoopVideo name={product.slug} label="" /></div><strong>{tx(product.name, lang)}</strong><span>{tx(roles[product.slug], lang)}<ArrowUpRight size={14} /></span></Link>)}
  </div>;
}
  ...
  return <>
    <section className="principle-section home-topic about-glow-preview" id="about">
      <div className="container principle-grid">
        ...
        <div className="about-brand-glow" aria-hidden="true"><LoopVideo name="about-glow" label="" className="about-glow-video" /><img className="about-fixed-logo" src={publicAsset("brand/eryndex-logo.webp")} alt="" /></div>
      </div>
  ...
  return <>
    ...
    <section className="hero-section hero-video-section">
      ...
      <svg className="hero-flow-overlay" viewBox="0 0 1600 800" preserveAspectRatio="none" aria-hidden="true">
        <defs>
          <path id="hero-flow-route-cyan" d="M-90 510 C150 490 300 360 480 400 S735 560 920 430 S1230 230 1690 305" />
          <path id="hero-flow-route-mint" d="M-100 335 C210 230 370 255 550 405 S850 580 1065 365 S1350 145 1700 220" />
          <path id="hero-flow-route-violet" d="M-70 690 C260 630 495 520 740 590 S1110 690 1360 480 S1570 380 1700 425" />
        </defs>
        <g className="hero-flow-traces">
          <path className="hero-flow-path flow-cyan" pathLength="100" d="M-90 510 C150 490 300 360 480 400 S735 560 920 430 S1230 230 1690 305" />
          <path className="hero-flow-path flow-mint" pathLength="100" d="M-100 335 C210 230 370 255 550 405 S850 580 1065 365 S1350 145 1700 220" />
          <path className="hero-flow-path flow-violet" pathLength="100" d="M-70 690 C260 630 495 520 740 590 S1110 690 1360 480 S1570 380 1700 425" />
        </g>
        <g className="hero-flow-core" transform="translate(1050 420)">
          <circle className="hero-flow-core-halo halo-outer" r="118" />
          <circle className="hero-flow-core-halo halo-middle" r="78" />
          <circle className="hero-flow-core-halo halo-inner" r="44" />
          <circle className="hero-flow-core-point" r="7" />
        </g>
        <g className="hero-flow-nodes">
          <circle className="flow-node node-cyan" cx="480" cy="400" r="4" />
          <circle className="flow-node node-mint" cx="550" cy="405" r="4" />
          <circle className="flow-node node-violet" cx="740" cy="590" r="4" />
          <circle className="flow-node node-cyan" cx="920" cy="430" r="3" />
          <circle className="flow-node node-mint" cx="1065" cy="365" r="3" />
          <circle className="flow-node node-violet" cx="1360" cy="480" r="3" />
        </g>
        <g className="hero-flow-runners">
          <circle className="flow-runner runner-cyan" r="5"><animateMotion dur="11s" begin="-3.4s" repeatCount="indefinite" rotate="auto"><mpath href="#hero-flow-route-cyan" xlinkHref="#hero-flow-route-cyan" /></animateMotion></circle>
          <circle className="flow-runner runner-cyan runner-secondary" r="3"><animateMotion dur="16s" begin="-11.1s" repeatCount="indefinite" rotate="auto"><mpath href="#hero-flow-route-cyan" xlinkHref="#hero-flow-route-cyan" /></animateMotion></circle>
          <circle className="flow-runner runner-mint" r="4.5"><animateMotion dur="13.5s" begin="-7.2s" repeatCount="indefinite" rotate="auto"><mpath href="#hero-flow-route-mint" xlinkHref="#hero-flow-route-mint" /></animateMotion></circle>
          <circle className="flow-runner runner-mint runner-secondary" r="2.8"><animateMotion dur="19s" begin="-2.8s" repeatCount="indefinite" rotate="auto"><mpath href="#hero-flow-route-mint" xlinkHref="#hero-flow-route-mint" /></animateMotion></circle>
          <circle className="flow-runner runner-violet" r="4"><animateMotion dur="15.5s" begin="-9.6s" repeatCount="indefinite" rotate="auto"><mpath href="#hero-flow-route-violet" xlinkHref="#hero-flow-route-violet" /></animateMotion></circle>
          <circle className="flow-runner runner-violet runner-secondary" r="2.6"><animateMotion dur="22s" begin="-5.2s" repeatCount="indefinite" rotate="auto"><mpath href="#hero-flow-route-violet" xlinkHref="#hero-flow-route-violet" /></animateMotion></circle>
        </g>
      </svg>
      <div className="container hero-layout">
        <div className="hero-copy"><h1>{lang === "en" ? <>Make everyday<br /><em>work clearer</em><br /><em>and easier to move forward</em></> : lang === "zh-CN" ? <>企业每天的工作<br /><em>更清晰</em><br /><em>更容易前进</em></> : <>企業每天的工作<br /><em>更清楚</em><br /><em>更容易前進</em></>}</h1><p className="hero-lede">{lang === "en" ? "Eryndex builds sustainable AI work systems for small and medium businesses. From workflow automation and data security to operational intelligence and internal AI, we help teams reduce repetition, make data context clearer, and keep time for work that needs judgment." : lang === "zh-CN" ? "Eryndex 为中小企业与成长型团队建立可持续的 AI 工作系统。从流程自动化、资料安全、运营洞察到企业内部 AI，协助团队减少重复作业、看清资料脉络，并把时间留给真正需要判断的工作。" : "Eryndex 為中小企業與成長型團隊建立可持續的 AI 工作系統。從流程自動化、資料安全、營運洞察到企業內部 AI，協助團隊減少重複作業、看清資料脈絡，並把時間留給真正需要判斷的工作。"}</p><p className="hero-support">{lang === "en" ? "There is no need to transform every system at once. Start with one clear work scenario, then connect it into an operating structure that fits your business." : lang === "zh-CN" ? "不必一次改造所有系统。可以先从一个明确的工作场景开始，再逐步连接成适合企业的工作架构。" : "不必一次改造所有系統。可以先從一個明確的工作場景開始，再逐步連結成適合企業的工作架構。"}</p><div className="hero-actions"><PrimaryButton href="/contact">{lang === "en" ? "Talk to Eryndex about your needs" : lang === "zh-CN" ? "与 Eryndex 讨论需求" : "與 Eryndex 討論需求"}</PrimaryButton><SecondaryButton href={`${import.meta.env.BASE_URL}#products`}>{lang === "en" ? "Explore products" : lang === "zh-CN" ? "探索产品" : "探索產品"}</SecondaryButton></div></div>
        <div className="hero-art"><div className="hero-orbit orbit-a"><span className="hero-signal-dot dot-green" /></div><div className="hero-orbit orbit-b"><span className="hero-signal-dot dot-violet" /></div><div className="hero-image-frame"><img src={publicAsset("brand/hero-fallback.png")} alt={lang === "en" ? "Abstract enterprise intelligence network with connected data signals" : lang === "zh-CN" ? "连接数据讯号的抽象企业智能网络" : "連結資料訊號的抽象企業智慧網路"} /><div className="hero-art-overlay" /></div><svg className="hero-signal-overlay" viewBox="0 0 800 520" preserveAspectRatio="none" aria-hidden="true"><path id="hero-route-cyan" className="hero-signal-path path-cyan" pathLength="100" d="M-30 378 C120 328 132 184 286 226 S470 370 610 226 S730 130 830 162" /><path id="hero-route-mint" className="hero-signal-path path-mint" pathLength="100" d="M-30 116 C112 156 172 320 314 286 S484 116 628 180 S742 318 830 278" /><path id="hero-route-violet" className="hero-signal-path path-violet" pathLength="100" d="M-24 444 C140 408 226 354 344 382 S542 468 824 350" /><circle className="hero-signal-node node-cyan" cx="286" cy="226" r="3" /><circle className="hero-signal-node node-mint" cx="314" cy="286" r="3" /><circle className="hero-signal-node node-violet" cx="542" cy="420" r="3" /><circle className="hero-signal-runner runner-cyan" r="3.2"><animateMotion dur="6.8s" begin="-1.2s" repeatCount="indefinite" rotate="auto"><mpath href="#hero-route-cyan" xlinkHref="#hero-route-cyan" /></animateMotion></circle><circle className="hero-signal-runner runner-cyan runner-secondary" r="2.1"><animateMotion dur="10.5s" begin="-6.4s" repeatCount="indefinite" rotate="auto"><mpath href="#hero-route-cyan" xlinkHref="#hero-route-cyan" /></animateMotion></circle><circle className="hero-signal-runner runner-mint" r="2.8"><animateMotion dur="8.4s" begin="-4.8s" repeatCount="indefinite" rotate="auto"><mpath href="#hero-route-mint" xlinkHref="#hero-route-mint" /></animateMotion></circle><circle className="hero-signal-runner runner-mint runner-secondary" r="1.9"><animateMotion dur="12.6s" begin="-1.7s" repeatCount="indefinite" rotate="auto"><mpath href="#hero-route-mint" xlinkHref="#hero-route-mint" /></animateMotion></circle><circle className="hero-signal-runner runner-violet" r="2.7"><animateMotion dur="9.7s" begin="-7.1s" repeatCount="indefinite" rotate="auto"><mpath href="#hero-route-violet" xlinkHref="#hero-route-violet" /></animateMotion></circle><circle className="hero-signal-runner runner-violet runner-secondary" r="1.8"><animateMotion dur="14.2s" begin="-3.9s" repeatCount="indefinite" rotate="auto"><mpath href="#hero-route-violet" xlinkHref="#hero-route-violet" /></animateMotion></circle></svg></div>
      </div>
    </section>
    <HomeAbout />
